...
export function getFallbackSEOContent(tool: ToolConfig): Partial<ToolConfig> {
  ...
  let examples = [
    { input: 'Standard user configuration / typical file upload', output: 'Formatted, calculated, or compiled representation' }
  ];
  const howToSteps = [
    { name: 'Enter Inputs', text: 'Input your data, values, or upload your file into the provided container panel.' },
    { name: 'Configure Options', text: 'Adjust sliders, selectors, or toggles to customize your desired output parameters.' },
    { name: 'Verify Live Preview', text: 'Review the instant calculation or visual representation generated in real-time.' },
    { name: 'Export Result', text: 'Click the Copy or Download button to save your formatted output.' }
  ];
  const faq = [
    {
      question: `Is my data shared when using the ${name}?`,
      answer: 'No. ToolPilot operates under a zero-server policy. All calculations, data formatting, and file exports are executed locally on your machine.'
    },
    {
      question: `Do I need to sign up or pay to use the ${name}?`,
      answer: 'No, this tool is 100% free. There are no limits, sign-ups, subscriptions, or hidden charges required.'
    }
  ];
  ...
  if (module === 'finance') {
    ...
    examples = [
      { input: 'Typical financial inputs (Rate: 10%, Period: 5 Years)', output: 'Accurate calculated return values and amortization rows' }
    ];
    faq.push({
      question: 'What is a reducing interest rate?',
      answer: 'A reducing interest rate means interest is charged only on the remaining outstanding principal balance at the end of each period, rather than the initial loan amount.'
    });
    ...
    faq.push({
      ...
      answer: 'Yes, absolutely. The tool executes 100% inside your local browser memory space. No inputs are sent to any servers.'
    });
    ...
    faq.push({
      ...
      answer: 'Export your image as WebP or PNG format. Standard JPEG format does not support transparent layers and will convert transparency to white.'
    });
  } else if (module === 'text') {
