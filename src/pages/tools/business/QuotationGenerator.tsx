...
export default function QuotationGenerator() {
  ...
  const [companyContact, setCompanyContact] = useState<string>(
    'hello@creativeagency.com | +91 98765 43210'
  );
  const [companyAddress, setCompanyAddress] = useState<string>(
    '404 Design Tower, Sector 62, Noida, UP'
  );

  const [clientName, setClientName] = useState<string>('Tech Solutions Inc.');
  const [clientContact, setClientContact] = useState<string>(
    'billing@techsolutions.com | +91 99999 88888'
  );
  const [clientAddress, setClientAddress] = useState<string>(
    '808 Tech Park, Whitefield, Bangalore'
  );
  ...
  const removeItem = (id: string) => {
    if (items.length <= 1) {
      setItems([
        { id: '1', description: 'New Service/Item', qty: 1, price: 0, discount: 0, tax: 18 },
      ]);
      return;
    }
            ...
            >
              <div>
                ...
                <p className="text-sm mt-1" style={{ color: 'var(--text-secondary)' }}>
                  Quote No: <span className="font-bold">{quoteNumber}</span>
                </p>
